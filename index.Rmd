...
acf(na.omit(log(GDP)), lag.max = 8, plot = FALSE)
```
...
```{r echo=TRUE, error=FALSE, warning=FALSE, out.width = "70%"}
# ACF of US quarterly GDP
acf(na.omit(log(GDP)), lag.max = 24, plot = TRUE)
```
...
```{r echo=TRUE, error=FALSE, warning=FALSE, out.width = "70%"}
# ACF of US quarterly GDP
acf(na.omit(GDPGrowth), lag.max = 24, plot = TRUE)
```
...
acf(na.omit(usunemp), lag.max = 24, plot = TRUE)
```
...
acf(na.omit(dollarpoundFX), lag.max = 24, plot = TRUE)
```
...
acf(na.omit(log(JPindprod)), lag.max = 24, plot = TRUE)
```
...
acf(na.omit(NYSESW), lag.max = 24, plot = TRUE)
```
